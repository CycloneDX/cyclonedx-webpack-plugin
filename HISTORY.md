...
* Changed
  * This tool supports _CycloneDX_ Specification-1.5 now ([#1001] via [#])
  * This tool warns now, if SBOM generation is skipped due to an unsupported value for option `specVersion` via [#])  
    Previous behaviour was a silent skip.
* Added
  * Option `specVersion` now supports value `1.5` to reflect _CycloneDX_ Specification-1.5 ([#1001] via [#])  
    Default value for that option is unchanged - still `1.4`.
* Misc
  * Raised dependency `@cyclonedx/cyclonedx-library@^3||^4`, was `@^2.0.0` ([#1001] via [#])
